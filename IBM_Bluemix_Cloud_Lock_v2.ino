...
void callback(char* topic, byte* payload, unsigned int length) {
 ...
 // Check to see if you have received a command you recognize.
  if (msgString.equals("{\"lightmode\": \"OFF\"}")) {
      ...
      digitalWrite(13, HIGH);     //turn ON LED to show Access Code verified
  } else if (msgString.equals("{\"lightmode\": \"YELLOW\"}")) {
      signal_mode = MODE_YELLOW;
      Serial.println("Signal mode is yellow");
  }
}
...
void changeLED() {
  ...
  else if (signal_mode == MODE_YELLOW) {
      for(i=0; i<NUMPIXELS; i++) {
